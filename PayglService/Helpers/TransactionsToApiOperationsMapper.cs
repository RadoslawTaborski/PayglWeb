...
namespace PayglService.Helpers
{
    internal class TransactionsToApiOperationsMapper
    {
        ...
        public ApiOperation Convert(Transaction transaction, ApiUser user, List<ApiImportance> importances, List<ApiFrequency> frequencies, List<ApiTag> tags, List<ApiTransactionType> transactionsType, List<ApiTransferType> transfersType)
        {
            ...
            {
                result = new ApiOperation(null, null, user, -1 * transaction.Amount, transactionsType[1], transfersType[1], null, null, transaction.DateTime.ToString("yyyy-MM-dd"), "", new List<ApiRelTag>().ToArray(), new List<ApiOperationDetails>().ToArray(), transaction.ContractorData + Environment.NewLine + transaction.Title);
            }
            else
            {
                result = new ApiOperation(null, null, user, transaction.Amount, transactionsType[0], transfersType[1], null, null, transaction.DateTime.ToString("yyyy-MM-dd"), "", new List<ApiRelTag>().ToArray(), new List<ApiOperationDetails>().ToArray(), transaction.ContractorData + Environment.NewLine + transaction.Title);
            }
